    ...
    <div className={`${className} slider-container w-full your_slide`}>
      <Slider {...settings}>
        {discover.map((category, index) => (
          ...
          >
            {category.islast ? (
              <div className="verflow-hidden rounded-xl">
                <img
                  src={category.img}
                  alt={category.name}
                ...
                >
                  <span className="text-xl hover:bg-black  hover:text-white transition-all duration-300 border-2  border-black bg-tranparent w-16 h-16 rounded-full flex items-center justify-center">
                    {category.icon}
                  </span>
